...
## Mark4：DeepSeek语义协调实验

仓库新增了完整独立的 [`Mark4/`](Mark4/) 实验。该版本按照《AI RAN博弈共识.pdf》的Algorithm 1和第3节验证指标，引入阿里GenTD26生产轨迹与DeepSeek官方API，实现真实的“Intent - LLM反馈 - 公式校验 - 最佳响应”循环。
...
| DeepSeek真实API记录 | 662 |
| Token总量 | 255,008 |
| 平均 / P95 API时延 | 1.309秒 / 1.866秒 |

Mark4包含四个基准、势函数收敛、能耗-时延帕累托候选、`N=30-200`显存违规率、LLM Token和时延，以及M/M/1与VRAM障碍拟合。详细公式、假设、输出路径和结论边界见 [`Mark4/README.md`](Mark4/README.md)。

> Mark4验证的是有限样本下的PSNE与软件仿真容量代理，不证明全局最优、真实硬件零OOM或部署可行性。实测LLM时延为秒级，不支持原PDF的微秒至毫秒级预期。
