...
诊断结果为 `R² = -0.0116`，说明匿名数据中的队列长度与时延关联很弱。这两个参数只能称为仿真代理，不能称为物理识别出的到达率和服务率。
...
拟合结果为 `alpha = 5.8080`、`beta = 11.0109`、`R² = 0.9918`。其中 `beta` 和尾部形状来自真实VRAM波动；16GB容量以及容量点障碍代价5.0属于明确假设。
...
```math
s_i=1\quad\text{if}\quad e_{i,\mathrm{tx}}+\Delta_{\mathrm{center},i}<e_{i,\mathrm{loc}}.
```
...
`pareto_front.csv`按局部卸载收益排序并枚举 `K = 0...100`，报告模拟能耗与全任务平均排队时延。它是一个可复核的候选前沿，不是遍历 `2^100` 种策略得到的全局帕累托前沿。

模拟能耗由10W边缘功率、本地1.8倍时长、无线每KB能耗等假设换算，不能当成Jetson实测值。

### 6.4 Memory Violation Rate

按PDF要求扫描 `N = 30...200`，每个 `N` 运行30个固定种子：
...
3. `03_memory_violation_rate`：`N = 30...200`容量违规率；
